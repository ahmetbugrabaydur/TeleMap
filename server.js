...
let sessionData = { 
    currentMap: null, 
    activeTokens: {},
    combatTracker: { list: [], turnIndex: 0 } 
};
...
io.on('connection', (socket) => {
    ...
    socket.emit('combatSync', sessionData.combatTracker);
    ...
    socket.on('combatUpdate', (data) => {
        sessionData.combatTracker = data;
        saveToDisk();
        socket.broadcast.emit('combatSync', data);
    });
    socket.on('chatMessage', (data) => {
        // Send it to everyone in the room except the person who rolled the message or the dice.
